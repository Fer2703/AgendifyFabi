  ...
  return (
    <div className="w-full h-full">
      ...
      <div className='w-80 h-16 transform left-1/2 -translate-x-1/2 top-20 absolute'>
        <motion.div 
          className='w-full h-8 top-0 absolute text-center text-green-900 text-3xl font-bold'
        ...
        <motion.div 
          className='w-full h-8 top-7 absolute text-center text-green-900 text-6xl font-bold'
      ...
      {/* TAREAS */}
      <div className={`w-[92%] h-[calc(100vh-14.25rem)] transform left-1/2 -translate-x-1/2 top-57 absolute overflow-y-auto px-2`}>
        <div className="flex flex-col gap-y-6">
          {/* Para finalizar */}
          {tareasParaFinalizar.length > 0 && (
            <div className='w-full'>
              <div className='w-full h-10 text-green-900 text-3xl font-bold py-6'>Para finalizar</div>
              <div className='flex flex-col gap-y-6'>
                {tareasParaFinalizar.map((task) => (
                  <div key={task.id} className='w-full h-48 bg-white rounded-3xl relative'>
                    <div className="w-60 h-10 left-5 top-4 absolute justify-start text-green-900 text-3xl font-bold">{task.title}</div>
                    ...
                    <div className="w-40 h-11 left-5 bottom-4 leading-none absolute justify-center text-green-900 text-2xl font-bold">DESDE: {task.from.slice(0,5)}<br/>HASTA: {task.to.slice(0,5)}</div>
                    <motion.div 
                      className={`w-40 h-11 right-2 bottom-4 absolute rounded-[19px] cursor-pointer ${task.completed ? 'bg-zinc-500' : 'bg-green-600'}`}
          ...
          {tareasParaTrabajar.length > 0 && (
            <div className='w-full'>
              <div className='w-full h-10 text-green-900 text-3xl font-bold py-6'>Para ir trabajando</div>
              <div className='flex flex-col gap-y-6'>
